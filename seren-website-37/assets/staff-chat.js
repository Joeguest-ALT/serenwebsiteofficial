...
(function () {
  ...
  function injectStyles() {
    ...
    const css = `
      ...
      .staff-name-input{width:200px;padding:11px 12px;font-size:16px;text-align:center;border:2px solid #0E2C53;border-radius:8px;outline:none;display:block;margin:0 auto 10px;font-family:inherit;color:#0E2C53;}
      .staff-name-input:focus{border-color:#FBBD1E;}
      .staff-pin-input{width:200px;padding:12px;font-size:16px;text-align:center;letter-spacing:4px;border:2px solid #0E2C53;border-radius:8px;outline:none;-webkit-text-security:disc;text-security:disc;}
  ...
  function enterPinMode() {
    ...
    screen.innerHTML = `
      <h3>Staff sign in</h3>
      <p>Enter your surname and payroll number.</p>
      <input type="text" class="staff-name-input" id="staff-surname-input" placeholder="Surname" autocomplete="family-name" spellcheck="false" />
      <input type="text" inputmode="numeric" pattern="[0-9]*" maxlength="6" minlength="2" class="staff-pin-input" id="staff-pin-input" placeholder="Payroll no." autocomplete="off" />
    ...
    setTimeout(() => $("#staff-surname-input")?.focus(), 50);
    $("#staff-surname-input")?.addEventListener("keydown", (e) => {
      if (e.key === "Enter") { e.preventDefault(); $("#staff-pin-input")?.focus(); }
    });
    ...
    $("#staff-pin-submit")?.addEventListener("click", async () => {
      ...
      const surEl = $("#staff-surname-input");
      const submit = $("#staff-pin-submit");
      const errEl = $("#staff-pin-error");
      const pin = input.value.trim();
      const surname = (surEl && surEl.value || "").trim();
      errEl.textContent = "";
      if (!surname) {
        errEl.textContent = "Enter your surname.";
        return;
      }
      if (!/^\d{2,6}$/.test(pin)) {
        errEl.textContent = "Enter your payroll number.";
        return;
      }
      submit.disabled = true;
      submit.textContent = "Checking…";
      try {
        const res = await callFunction({ action: "verify_pin", pin, surname });
        if (res.ok) {
          storeToken(res.token, res.expiresAt);
          localStorage.setItem("seren_staff_name", res.staff?.name || "");
          enterStaffMode();
        } else {
          errEl.textContent = res.error || "Surname and payroll number do not match.";
